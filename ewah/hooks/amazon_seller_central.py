...
class EWAHAmazonSellerCentralHook(EWAHBaseHook):
    ...
    _REPORT_METADATA = {
        ...
        "settlement_report_v2_financial_transactions": {
    ...
    def get_settlement_report_v2_data(
        ...
        """Fetch settlement report v2 data from Amazon Seller Central.

        This report contains detailed financial transaction data including:
        - Settlement ID
        - Transaction type
        - Order ID
        - Posted date
        - Other financial details (specifically, all fee types)

        ! Note:
        Unlike the other report types in this hook, the settlement report works differently:
        1. Settlement reports are automatically generated by Amazon on a regular basis
        2. Instead of requesting a new report to be generated, we search for and download
           existing reports that were already created by Amazon
        3. The date range parameters (data_from and data_until) are used to find reports
           that were created within that period, not to generate new reports
        4. Multiple reports may be returned if they exist in the specified date range
        5. Each report contains a complete settlement period's worth of data
        6. There is a 90-day limit on how far back you can retrieve reports
        7. The first row of the report is a summary row that has no transaction details
        and has to be skipped in order to have a consistent data structure that allows for
        incremental loading via the "posted-date" field (which is NULL for summary rows
        -> cannot be parsed as datetime object).

        This is why this endpoint uses a different approach than other report types:
        - It uses GET instead of POST to search for existing reports
        - It processes multiple reports if they exist in the date range
        - It doesn't require report generation, just report retrieval
        """
        self.log.info(
        ...
        # Settlement reports require a data_from parameter
        assert data_from, "Requires a minimum date parameter!"

        # Ensure data_until is not None
        if not data_until:
            data_until = datetime.utcnow().replace(tzinfo=pytz.utc)
        ...
        if data_from < min_possible_date:
            self.log.info(
                ...
                f"Adjusting to {min_possible_date} to comply with API limitations."
            )
            data_from = min_possible_date
        ...
        # Step 2: Search for available reports
        ...
        self.log.info(
            f"Searching for settlement reports created between {data_from.isoformat()} "
            f"and {data_until.isoformat()}..."
        )
        max_retries = 3
        delay = 60
        ...
        if not reports:
            self.log.warning("No settlement reports found in the specified date range")
            return

        self.log.info(f"Found {len(reports)} settlement reports")
        ...
        for report_index, report in enumerate(reports, 1):
            ...
            self.log.info(f"Report {report_id} - Step 2/3: Got document URL")
            ...
            # Reset the CSV reader to start from beginning
            data_io.seek(0)
            csv_reader = csv.DictReader(data_io, delimiter="\t")
            ...
            for row in csv_reader:
                ...
                data.append(row)
                if i == batch_size:
                    self.log.info(f"Yielding batch of {len(data)} rows.")
                    yield data
                    i = 0
                    data = []

            # Yield any remaining transaction data that hasn't been yielded yet due to batch size
            if data:
                yield data
