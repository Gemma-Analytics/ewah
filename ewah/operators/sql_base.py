...
class EWAHSQLBaseOperatorOld(EWAHBaseOperator):
    # Don't overwrite the _NAMES list to avoid accidentally exposing the SQL
    # base operator
    # _NAMES = []
    ...
    def __init__(
        ...
        **kwargs
                ...
                if not chunking_column and not kwargs.get("primary_key_column_name"):
                    ...
                    for key in columns.keys():
                        if columns[key].get(EC.QBC_FIELD_PK):
                            ...
                            break
                chunking_column = chunking_column or kwargs["primary_key_column_name"]

            else:
                raise Exception(
                    "Arg chunking_interval must be integer or " + "datetime.timedelta!"
                )
        ...
        if kwargs.get("reload_data_from") and not chunking_interval:
            raise Exception(
                "When setting reload_data_from, must also set " + "chunking_interval!"
            )
        ...
        if (
            ...
            or use_execution_date_for_incremental_loading
        ):
            if not timestamp_column:
                raise Exception(
                    "If you used data_from and/or data_until, you"
        ...
        if sql_select_statement:
            err_msg = "sql_select_statement and {0} cannot" " be used in combination!"
    ...
    def ewah_execute(self, context):
        ...
        if self.extract_strategy == EC.ES_FULL_REFRESH:
            ...
            self.log.info(
                "Incrementally loading data from {0} to {1}.".format(
        ...
        if self.chunking_interval and chunking_column:
            ...
            while previous_chunk <= max_chunk:
                ...
                data = self._get_data_from_sql(
                    sql=sql_base.format(self._SQL_CHUNKING_CLAUSE).format(
                        column=chunking_column,
                        equal_sign=(
                            "="
                            if max_chunk < (previous_chunk + self.chunking_interval)
                            else ""
                        ),
                    ),
                    params=params,
