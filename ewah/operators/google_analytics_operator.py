...
class EWAHGAOperator(EWAHBaseOperator):
    ...
    *args, **kwargs):
        ...
        if kwargs.get('reload_data_from') and not (reload_data_chunking or chunking_interval):
    ...
    def ewah_execute(self, context):
        if not self.test_if_target_table_exists():
            self.chunking_interval = self.reload_data_chunking \
                ...
                or (self.load_data_until - self.load_data_from)
        elif not self.chunking_interval:
            self.chunking_interval = self.load_data_until - self.load_data_from
        ...
        while data_from < data_until:
            ...
            self.since_formatted = data_from.strftime('%Y-%m-%d')
            self.until_formatted = min(
                    data_from + self.chunking_interval,
                    data_until,
                ).strftime('%Y-%m-%d')
