...
class EWAHMongoDBOperator(EWAHBaseOperator):
        ...
        **kwargs
        ...
        if chunking_interval:
            ...
            if not chunking_field:
                raise Exception(
                    "if chunking_interval is set, must also set "
        ...
        if pagination_limit:
            ...
            if not kwargs.get("primary_key_column_name"):
                raise Exception(
                    "pagination_limit must be used with " + "primary_key_column_name!"
                )
    ...
    def extract_and_load_paginated(
        self,
        collection,
    ...
    def ewah_execute(self, context):
        ...
        if self.chunking_interval:
            ...
            self.log.info(
                "Getting data range using:\n{0}".format(str(aggregation_statement))
            )
            ...
            while current_val < max_val:
                ...
                self.log.info(
                    "Getting data from {0} to {1}...".format(
                        str(current_val),
                        str(next_val),
                    )
                )
                ...
                self.log.info("Filter expression: {0}".format(str(filter_expressions)))
