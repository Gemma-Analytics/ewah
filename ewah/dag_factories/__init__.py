...
def dags_from_dict(
    ...
    def check_keys_are_allowed(dict, allowed_list, dict_name):
        for key in dict.keys():
            if not key in allowed_list:
                warn_me(
                    "Unexpected key {0} was found in dict {1}!".format(
                        key,
                        dict_name,
                    )
                )
    ...
    for key, value in additional_dag_args.items():
        if key in allowed_dag_args:
            ...
            warn_me(
                (
                    "Unexpected key {0} found in "
                    + "additional_dag_args in base_config!"
                ).format(
                    key,
                )
            )
    ...
    for key, value in additional_task_args.items():
        if key in allowed_task_args:
            ...
            warn_me(
                "Unexpected key {0} found in additional_task_args!".format(
                    key,
                )
            )
    ...
    for dag_name, dag_config in dags_dict.items():
        ...
        config["additional_dag_args"].update(dag_config.pop("additional_dag_args", {}))
        ...
        try:
            ...
            raise Exception(
                "Invalid dwh_engine {0}\nAllowed:\n\t{1}".format(
                    str(config.get("dwh_engine", None)),
                    "\n\t".join(list(dwh_engines.keys())),
                )
            )
        try:
            if type(config["el_operator"]) == type and issubclass(
                config["el_operator"], EBO
            ):
